...
def getData():
    ...
    if (not (option == 'Select a Question')):
        ...
        st.write(keywords)
        if st.button("Modify keywords"):
            modifyKeywords(keywords,model_answer)
        ...
        user_input = st.text_input("Enter associated keyword",'')
        if(user_input != ''):
            qword = user_input.lower()
            option1 = option.lower()
            if (qword not in option1):
                st.write("Invalid keyword, keyword must be present in the question")
            else:
                ...
                if img_file:
                    ...
                    g_fac /= 100
                    s_fac /= 100
                    # score = processAns(option,docText,user_input.lower(),keywords,g_fac,s_fac)
